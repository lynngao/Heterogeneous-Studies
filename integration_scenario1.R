...
#command_args <- c(1,1,100,10,1.1,"logit",
...
ed = as.numeric(command_args[5])
method = as.character(command_args[6])

count_data1 <- readRDS(command_args[7]) #Asian population
count_data2 <- readRDS(command_args[8]) #Caucacian population
...
pseudo_prob <- function(alpha, prob1, prob2, prob3, num_gene, ed, otu_selected){
  ...
  prob_v2_case[otu_enrich] = prob_v2_ctr[otu_enrich]*ed
  prob_v2_case[otu_deplete] = prob_v2_ctr[otu_deplete]/ed
  
  prob_v3_case = prob_v3_ctr
  prob_v3_case[otu_enrich] = prob_v3_ctr[otu_enrich]*ed
  prob_v3_case[otu_deplete] = prob_v3_ctr[otu_deplete]/ed
  
  prob_v1_case = prob_v1_case/sum(prob_v1_case)
  prob_v2_case = prob_v2_case/sum(prob_v2_case)
  prob_v3_case = prob_v3_case/sum(prob_v3_case)
  return(list(prob_v1_ctr, prob_v2_ctr, prob_v3_ctr, prob_v1_case, prob_v2_case, prob_v3_case))
}
...
prob_v = pseudo_prob(alpha, prob_data1, prob_data2, prob_data3, num_gene, ed, otu_selected)
prob_v1_ctr = prob_v[[1]]
prob_v2_ctr = prob_v[[2]]
prob_v3_ctr = prob_v[[3]]
prob_v1_case = prob_v[[4]]
prob_v2_case = prob_v[[5]]
prob_v3_case = prob_v[[6]]
...
for (i in 1:100) {
  ...
  simulation2 = as.data.frame(generate_samples(alpha, lambda, prob_v2_ctr, prob_v2_case, count_data2, sample_size)) #training set2
  simulation3 = as.data.frame(generate_samples(alpha, lambda, prob_v3_ctr, prob_v3_case, count_data3, sample_size)) #testing set
...
####  Output results
file_name <- sprintf('scenario1_%s_auc_a%s_l%s_s%s_g%s_e%s_test%s_logRelAbun.csv', 
                     gsub('.', '', method, fixed=T),
                     gsub('.', '', alpha, fixed=T), gsub('.', '', lambda, fixed=T), 
                     gsub('.', '', sample_size, fixed=T), gsub('.', '', num_gene, fixed=T), gsub('.', '', ed, fixed=T), 
